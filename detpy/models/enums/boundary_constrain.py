...
def boundary_projection_to_midpoint(member: Member):
    ...
    if np.any((x < lb) | (x > ub)):
        ...
        for j in range(len(x)):
            ...
            if d > 0:
                alpha_j = (ub[j] - midpoint[j]) / d
            else:
                alpha_j = (lb[j] - midpoint[j]) / d
...
def boundary_penalty_death(member: Member):
    ...
    if any(ch.real_value < ch.lb or ch.real_value > ch.ub for ch in member.chromosomes):
...
def boundary_resampling(trial, population, fitness_fun, base_vector_schema, optimization_type, mutation_y, mutation_f):
    ...
    for member_trial, member_parent in zip(trial.members, population.members):
        ...
        for idx, (member_trial, member_parent) in enumerate(zip(trial.members, population.members)):

            #10 time we try to generate feasible member
            for _ in range(10):
                is_not_valid = any(
                    ch.real_value < ch.lb or ch.real_value > ch.ub
                    for ch in member_trial.chromosomes
                )

                if not is_not_valid:
                    break

                #Choosing of base vector
                ...
                if base_vector_schema == BaseVectorSchema.RAND:
                    ...
                    base_vector = get_best_member(optimization_type, population)
                else:
                    raise ValueError("Unknown base vector schema.")

                #Mutation and overrride of original memeber
                member_trial = mutation_ind(base_vector, diff_members, mutation_f)
                trial.members[idx] = member_trial

            #After 10 times we check if it is feasible
            is_not_valid = any(
                ch.real_value < ch.lb or ch.real_value > ch.ub
                for ch in member_trial.chromosomes
            )
            if is_not_valid:
                #Lamarckian repair (projection)
...
def boundary_projection_to_base(member_trial: Member, base_vector: Member):
    ...
    for i, ch in enumerate(member_trial.chromosomes):
        ch.real_value = m[i]
